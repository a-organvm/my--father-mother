...
def decrypt_db_bytes(payload: bytes, key: bytes) -> bytes:
    ...
    try:
        ...
        raise RuntimeError(
            "failed to decrypt DB; Keychain key may be missing or wrong"
        ) from e
...
def read_keychain_password(service: str, account: str) -> Optional[str]:
    if platform.system() != "Darwin" or not shutil.which("security"):
        raise RuntimeError(
            "encrypted DB keys require macOS Keychain via the 'security' command"
        )
    proc = subprocess.run(
...
def sync_pull(target: str) -> tuple[bool, str]:
    ...
    if was_encrypted and not is_encrypted_db_file(DB_PATH):
        try:
            ...
            return (
                False,
                f"pull failed: re-encrypt failed; restored previous encrypted DB ({e})",
            )
            ...
            try:
                ...
                if backup and backup.exists():
                    ...
                    try:
                        ...
                        pass
                raise RuntimeError(
                    f"re-encrypt failed; restored previous encrypted DB ({e})"
                ) from e
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_init.add_argument(
        "--encrypt",
        action="store_true",
        help="store mfm.db encrypted with an AES-256-GCM key in Keychain",
    )
