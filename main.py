...
def validate_gumroad_license(
    conn: sqlite3.Connection, license_key: str
) -> Optional[bool]:
...
def apply_gumroad_license_event(
...
def cmd_watch(args: argparse.Namespace) -> None:
    ...
    embedder_choice, embedder_warning = resolve_embedder_for_use(
        conn, getattr(args, "embedder", None)
    )
    if embedder_warning:
...
def cmd_semantic_search(args: argparse.Namespace) -> None:
    conn = connect_db()
    init_db(conn)
    embedder_kind, embedder_warning = resolve_embedder_for_use(
        conn, getattr(args, "embedder", None)
    )
    if embedder_warning:
        say(FATHER, embedder_warning)
...
def cmd_settings(args: argparse.Namespace) -> None:
    ...
    say(
        FATHER,
        f"  license_status={format_setting_value(account.get('license_status'))}",
    )
    say(
        FATHER,
        f"  has_license_key={format_setting_value(account.get('has_license_key'))}",
    )
...
def cmd_config(args: argparse.Namespace) -> None:
    ...
    if args.get:
        ...
        if key == "max_bytes":
            ...
            say(
                FATHER,
                f"gumroad_webhook_secret={'***set***' if gumroad_webhook_secret(conn) else ''}",
            )
        elif key == "gumroad_permalink":
            say(
                FATHER,
                f"gumroad_permalink={get_setting(conn, 'gumroad_permalink', '')}",
            )
    ...
    if args.set:
        ...
        if key == "max_bytes":
            ...
            if requested_embedder == "e5-small" and not is_pro_enabled(conn):
                say(
                    FATHER,
                    f"e5-small embeddings require Pro. Upgrade: {get_upgrade_url(conn)}",
                )
            else:
...
"""
            ...
            self._send(
                200,
                {
                    ...
                    "gumroad_webhook_secret": (
                        "***" if gumroad_webhook_secret(conn) else ""
                    ),
                    "sync_target": get_setting(conn, "sync_target", ""),
                    "sync_interval": get_sync_interval(conn),
        ...
        if path == "/semantic_search":
            ...
            if requested_embedder == "e5-small" and not is_pro_enabled(conn):
                ...
                return
            embedder_kind, _embedder_warning = resolve_embedder_for_use(
                conn, qs.get("embedder", [None])[0]
            )
    ...
    def do_POST(self) -> None:
        ...
        if path == "/config":
            ...
            if "pro_enabled" in data:
                parsed = (
                    data["pro_enabled"]
                    if isinstance(data["pro_enabled"], bool)
                    else parse_bool_value(str(data["pro_enabled"]))
                )
                if parsed is None:
            ...
            if "gumroad_webhook_secret" in data:
                set_setting(
                    conn, "gumroad_webhook_secret", str(data["gumroad_webhook_secret"])
                )
                updated["gumroad_webhook_secret"] = (
                    "***" if gumroad_webhook_secret(conn) else ""
                )
            if "gumroad_permalink" in data:
                set_setting(conn, "gumroad_permalink", str(data["gumroad_permalink"]))
                updated["gumroad_permalink"] = get_setting(
                    conn, "gumroad_permalink", ""
                )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    group_cfg.add_argument(
        "--get",
        help="get a key (max_bytes, allow_secrets, max_db_mb, notify, pro_enabled, license_key, gumroad_* settings, embedder, cap_by_app, cap_by_tag, evict_mode, allow_pdf, allow_images, auto_summary_cmd, auto_tag_cmd, helper_*_cmd, sync_target, sync_interval, backup_* settings, ai_recall_cmd, ai_fill_cmd)",
    )
    group_cfg.add_argument(
        "--set",
        nargs=2,
        metavar=("KEY", "VALUE"),
        help="set a key (max_bytes, allow_secrets, max_db_mb, notify, pro_enabled, license_key, gumroad_* settings, embedder, cap_by_app, cap_by_tag, evict_mode, allow_pdf, allow_images, auto_summary_cmd, auto_tag_cmd, helper_*_cmd, sync_target, sync_interval, backup_* settings, ai_recall_cmd, ai_fill_cmd)",
    )
